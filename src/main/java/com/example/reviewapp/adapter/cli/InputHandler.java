...
public class InputHandler {
    ...
    private static final String K_MIN_RATING  = "minrating";
    /** Filter key for maximum rating (1..5 inclusive). */
    private static final String K_MAX_RATING  = "maxrating";
    /** Filter key for author substring match (case-insensitive). */
    private static final String K_AUTHOR      = "author";
    ...
    private static final String K_PRODUCT     = "productname";
    ...
    private static final String K_SORT_DATE   = "sortbydate";
    /** Sort flag: sort by rating when {@code true}. */
    private static final String K_SORT_RATING = "sortbyratings";
    ...
    private static final String K_END_DATE    = "enddate";
    /**
     ...
     */
    private static final String K_START_TIME  = "starttime";
    /**
            ...
            while (true) {
                ...
                try {
                    if (equalsIgnoreCase(command, "exit")) {
                        ...
                        // Intentionally commented out: enable when a paged/streaming API is ready
                        // List<Review> reviews = reviewService.getAllReviews();
                        // reviews.forEach(System.out::println);
                    } else if (command.startsWith("id ")) {
                        handleById(command.substring(3).trim());
                    } else if (command.startsWith("filter ")) {
                        // Intentionally commented out: wire to paged/filter endpoint when available
                        // Filters filters = parseInputFilters(command.substring(7).trim());
                        // reviewService.getFilteredReviewsPage(filters).forEach(System.out::println);
     ...
     * <p>
     * <b>Accepted keys:</b> see the {@code K_*} constants (e.g., {@link #K_RATING}, {@link #K_MIN_RATING}, etc.).<br>
    ...
    private Filters parseInputFilters(String criteria) {
        ...
        for (String rawPart : criteria.split(",")) {
            ...
            try {
                switch (key) {
                    case K_RATING:      builder.setRating(parseIntInRange(value, 1, 5, "rating")); break;
                    case K_MIN_RATING:  builder.setMinRating(parseIntInRange(value, 1, 5, "minrating")); break;
                    case K_MAX_RATING:  builder.setMaxRating(parseIntInRange(value, 1, 5, "maxrating")); break;
                    case K_AUTHOR:      builder.setAuthorName(value); break;
                    case K_TITLE:       builder.setReviewTitle(value); break;
                    case K_PRODUCT:     builder.setProductName(value); break;
                    case K_STORE:       builder.setStoreName(value); break;
                    case K_DATE:        builder.setReviewDate(parseDateStrict(value)); break;
                    case K_START_DATE:  builder.setStartDate(parseDateStrict(value)); break;
                    case K_END_DATE:    builder.setEndDate(parseDateStrict(value)); break;
                    case K_START_TIME:  builder.setStartTime(parseTimeLenient(value)); break; // effective only if model has time
                    case K_END_TIME:    builder.setEndTime(parseTimeLenient(value)); break;
                    case K_SORT_DATE:   builder.setSortByDate(Boolean.parseBoolean(value)); break;
                    case K_SORT_RATING: builder.setSortByRating(Boolean.parseBoolean(value)); break;
                    default:
    ...
    private void printHelp() {
        ...
        println("  filter <criteria>   - Filter reviews (e.g., author=John," + K_MIN_RATING + "=3)");
    ...
    private void printFilters() {
        ...
        println(" " + K_RATING      + "       : Integer  [1..5]         e.g., " + K_RATING + "=5");
        println(" " + K_MIN_RATING  + "   : Integer  [1..5]         e.g., " + K_MIN_RATING + "=3");
        println(" " + K_MAX_RATING  + "   : Integer  [1..5]         e.g., " + K_MAX_RATING + "=5");
        println(" " + K_AUTHOR      + "        : String                 e.g., " + K_AUTHOR + "=John Doe");
        println(" " + K_TITLE       + "         : String                 e.g., " + K_TITLE + "=iPhone 13");
        println(" " + K_PRODUCT     + "   : String                 e.g., " + K_PRODUCT + "=iPhone 13");
        println(" " + K_DATE        + "          : YYYY-MM-DD            e.g., " + K_DATE + "=2018-01-01");
        println(" " + K_STORE       + "         : String                 e.g., " + K_STORE + "=Amazon");
        println(" " + K_SORT_DATE   + "    : Boolean                e.g., " + K_SORT_DATE + "=true");
        println(" " + K_SORT_RATING + " : Boolean                e.g., " + K_SORT_RATING + "=false");
        println(" " + K_START_DATE  + "    : YYYY-MM-DD            e.g., " + K_START_DATE + "=2018-01-01");
        println(" " + K_END_DATE    + "      : YYYY-MM-DD            e.g., " + K_END_DATE + "=2018-12-31");
        println(" " + K_START_TIME  + "    : HH:mm[:ss] (effective only if review time exists)");
        println(" " + K_END_TIME    + "      : HH:mm[:ss] (effective only if review time exists)");
        println("Example: filter " + K_RATING + "=5," + K_STORE + "=Amazon," + K_DATE + "=2018-01-01");
    }
