...
public class ReviewService {
    ...
    public Review getReviewById(long reviewId) {
        if (reviewId <= 0) {
            throw new InvalidInputException("Review ID must be positive");
        }
    ...
    public List<Review> getReviewsByKeywords(List<String> keywords) {
        if (keywords == null || keywords.isEmpty()) {
            throw new InvalidInputException("Keywords must not be empty");
        }
        for (String keyword : keywords) {
            if (keyword == null || keyword.trim().isEmpty()) {
                throw new InvalidInputException("Keyword must not be empty");
            }
        }
    ...
    public List<Review> getReviewsPage(int page, int pageSize) {
        if (page <= 0) {
            throw new InvalidInputException("Page index must be positive");
        }
        if (pageSize <= 0) {
    ...
    public List<Review> getFilteredReviewsPage(Filters filters, int page, int pageSize) {
        if (filters == null) {
            throw new InvalidInputException("Filters must not be null");
        }
        if (page <= 0) {
            throw new InvalidInputException("Page index must be positive");
        }
    ...
    public List<Review> getAllReviews() {
        return queryPort.getAllReviews();
    }
    ...
    public void saveReviews(List<Review> reviews) {
        if (writePort == null) {
            throw new InvalidInputException("Write port not configured for ReviewService");
        }
        if (reviews == null || reviews.isEmpty()) {
            throw new InvalidInputException("Reviews list must not be empty");
        }
        for (Review review : reviews) {
            if (review == null) {
                throw new InvalidInputException("Review must not be null");
            }
        }
